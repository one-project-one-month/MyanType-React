...
const CompeteForm = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setChallengeId(response.data.challengeId); // Assuming response includes challengeId
      setIsWaiting(true); // Show waiting message
      setOpen(false);
  ...
  // Poll for challenge acceptance status
  useEffect(() => {
    let interval;
    if (isWaiting && challengeId) {
      interval = setInterval(async () => {
        try {
          const response = await api.get(`http://localhost:8181/compete/status/${challengeId}`);
          const { isAccepted } = response.data;
          if (isAccepted) {
            setIsWaiting(false);
            clearInterval(interval);
            toast.success(`Challenge accepted by ${username}!`, {
              position: 'top-center',
              style: {
                backgroundColor: '#141723',
                color: '#F4F4F5',
            ...
            // Optionally redirect to competition page or update UI
          }
        } catch (error) {
          console.error('Error checking challenge status:', error);
          setIsWaiting(false);
          clearInterval(interval);
          toast.error('Error checking challenge status. Please try again.', {
            position: 'top-center',
            style: {
              backgroundColor: '#141723',
              color: '#F4F4F5',
  ...
  }, [isWaiting, challengeId, username]);
  ...
  return (
    ...
    >
      {isWaiting ? (
        ...
        >
          Waiting for {username} to accept the challenge...
        </div>
      ) : (
        <>
          <h2
